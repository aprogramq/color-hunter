use palette::{IntoColor, Okhsl, Oklab, Oklch, Srgb};
...
impl CountConstraint {
    pub fn normalize(&self, count: u8) -> u8 {
        match self {
            Self::Fixed(value) => *value,
            Self::Range(range) => count.clamp(*range.start(), *range.end()),
        }
    }

    pub fn contains(&self, count: u8) -> bool {
        match self {
            Self::Fixed(value) => count == *value,
...
impl Generator {
    pub const ALL: [Generator; 6] = [
        ...
        Generator::Analogus,
        Generator::Triadic,
        Generator::SplitComplementary,
        Generator::Coolors,
    ];
    pub fn count_constraint(self) -> CountConstraint {
        match self {
            Generator::Coolors => CountConstraint::Fixed(5),

            Generator::Analogus => CountConstraint::Range(2..=12),

            Generator::Complementary
            | Generator::Triadic
            | Generator::SplitComplementary
            | Generator::ColorHunter => CountConstraint::Range(1..=12),
        }
    }

    pub fn normalize_count(self, count: u8) -> u8 {
        self.count_constraint().normalize(count)
    }
}

pub struct ColorHunter;
impl ColorHunter {
    pub fn generate_palette(count: u8) -> Vec<Srgb> {
        Self::generate_with_rng(count, &mut rand::rng())
    }

    fn generate_with_rng<R: Rng + ?Sized>(count: u8, rng: &mut R) -> Vec<Srgb> {
        if count == 0 {
            return Vec::new();
        }

        let template = Coolors::GENERATED[rng.random_range(0..Coolors::GENERATED.len())];
        let mut palette: Vec<Okhsl> = template
            .iter()
            .map(|hex| Coolors::hex_to_srgb(hex).into_color())
            .collect();

        let mean_lightness =
            palette.iter().map(|color| color.lightness).sum::<f32>() / palette.len() as f32;
        let hue_shift = rng.random_range(-12.0..12.0);
        let lightness_shift = rng.random_range(-0.025..0.025);
        let contrast_scale = rng.random_range(0.94..1.06);
        let saturation_scale = rng.random_range(0.90..1.02);

        for color in &mut palette {
            color.hue += hue_shift;
            color.lightness = (mean_lightness
                + (color.lightness - mean_lightness) * contrast_scale
                + lightness_shift)
                .clamp(0.0, 1.0);
            color.saturation = (color.saturation * saturation_scale).clamp(0.0, 1.0);
        }

        Self::resize_palette(&mut palette, count as usize);
        palette.into_iter().map(IntoColor::into_color).collect()
    }

    fn resize_palette(palette: &mut Vec<Okhsl>, count: usize) {
        if count == 1 {
            palette.drain(..2);
            palette.truncate(1);
            return;
        ...
        while palette.len() > count {
            let remove_index = (0..palette.len())
                .min_by(|&left, &right| {
                    Self::nearest_distance(palette, left)
                        .total_cmp(&Self::nearest_distance(palette, right))
                })
                .unwrap();
            palette.remove(remove_index);
        }

        if palette.len() < count {
            Self::expand_palette(palette, count);
        }

        while palette.len() < count {
            let insert_after = (0..palette.len() - 1)
                .max_by(|&left, &right| {
                    Self::distance(palette[left], palette[left + 1])
                        .total_cmp(&Self::distance(palette[right], palette[right + 1]))
                })
                .unwrap();
            let middle = Self::mix(palette[insert_after], palette[insert_after + 1]);
            palette.insert(insert_after + 1, middle);
        }
    }

    fn expand_palette(palette: &mut Vec<Okhsl>, count: usize) {
        let anchors = palette.clone();
        let dominant_hue = anchors
            .iter()
            .max_by(|left, right| left.saturation.total_cmp(&right.saturation))
            .unwrap()
            .hue
        ...
        let mut selected: Vec<(f32, Okhsl)> = anchors
            .iter()
            .copied()
            .enumerate()
            .map(|(index, color)| (index as f32, color))
            .collect();
        let mut candidates = Vec::with_capacity(anchors.len() * variants.len());

        for (anchor_index, anchor) in anchors.iter().copied().enumerate() {
            for (variant_index, (hue_shift, lightness_shift, saturation_scale, order)) in
                variants.iter().copied().enumerate()
            {
                let is_neutral = anchor.saturation < 0.08;
                let hue = if is_neutral {
                    dominant_hue + hue_shift
                } else {
                    anchor.hue.into_positive_degrees() + hue_shift
                };
                let saturation = if is_neutral {
                    0.12 + variant_index as f32 * 0.025
                } else {
                    anchor.saturation * saturation_scale + 0.02
                };
                let candidate = Okhsl::new(
                    hue,
                    saturation.clamp(0.05, 0.92),
                    (anchor.lightness + lightness_shift).clamp(0.04, 0.96),
                );
                candidates.push((anchor_index as f32 + order, candidate));
            }
        }
            ...
            let best_index = (0..candidates.len())
                .max_by(|&left, &right| {
                    let left_distance = selected
                        .iter()
                        .map(|(_, color)| Self::distance(*color, candidates[left].1))
                        .fold(f32::INFINITY, f32::min);
                    let right_distance = selected
                        .iter()
                        .map(|(_, color)| Self::distance(*color, candidates[right].1))
                        .fold(f32::INFINITY, f32::min);
        ...
        *palette = selected.into_iter().map(|(_, color)| color).collect();
    }

    fn nearest_distance(palette: &[Okhsl], index: usize) -> f32 {
        let left = index
            .checked_sub(1)
            .map(|neighbor| Self::distance(palette[index], palette[neighbor]));
        let right = palette
            .get(index + 1)
            .map(|neighbor| Self::distance(palette[index], *neighbor));
    ...
    fn distance(first: Okhsl, second: Okhsl) -> f32 {
        let first: Oklab = first.into_color();
        let second: Oklab = second.into_color();
        let delta_l = first.l - second.l;
        let delta_a = first.a - second.a;
        let delta_b = first.b - second.b;

        (delta_l * delta_l + delta_a * delta_a + delta_b * delta_b).sqrt()
    }

    fn mix(first: Okhsl, second: Okhsl) -> Okhsl {
        let first_hue = first.hue.into_positive_degrees();
        let second_hue = second.hue.into_positive_degrees();
        let hue_delta = (second_hue - first_hue + 540.0).rem_euclid(360.0) - 180.0;

        Okhsl::new(
            first_hue + hue_delta * 0.5,
            (first.saturation + second.saturation) * 0.5,
            (first.lightness + second.lightness) * 0.5,
        )
    }
}

fn gamut_map(color: Oklch) -> Srgb {
    let lightness = color.l.clamp(0.02, 0.98);
    let chroma = color.chroma.max(0.0);
    let candidate = Oklch::new(lightness, chroma, color.hue);
    let rgb: Srgb = candidate.into_color();
    ...
    for _ in 0..16 {
        ...
        let rgb: Srgb = Oklch::new(lightness, middle, color.hue).into_color();
        if is_in_srgb_gamut(&rgb) {
            lower = middle;
        } else {
            upper = middle;
    ...
    Oklch::new(lightness, lower, color.hue).into_color()
}

fn is_in_srgb_gamut(color: &Srgb) -> bool {
...
impl Complementary {
    pub fn generate_palette(count: u8) -> Vec<Srgb> {
        ...
        let base_color = Oklch::new(
            rng.random_range(0.35..0.75),
            rng.random_range(0.12..0.22),
            rng.random_range(0.0..360.0),
        );

        Self::generate_colors(base_color, count)
            .into_iter()
            .map(gamut_map)
            .collect()
    }

    fn generate_colors(base: Oklch, count: u8) -> Vec<Oklch> {
        if count == 0 {
            return Vec::new();
        }
        ...
        (0..count)
            .map(|index| Oklch::new(base.l, base.chroma, base.hue + hue_step * index as f32))
            .collect()
    }
}
...
impl Analogus {
    pub fn generate_palette(count: u8) -> Vec<Srgb> {
        let mut rng = rand::rng();
        let base_color = Oklch::new(
            rng.random_range(0.45..0.7),
            rng.random_range(0.1..0.16),
            rng.random_range(0.0..360.0),
        );
    ...
    fn generate_colors(base: Oklch, count: u8) -> Vec<Oklch> {
        match count {
            ...
            1 => vec![base],
            _ => {
                let hue_step = 90.0 / (count - 1) as f32;
                let start_hue = base.hue - 45.0;
                (0..count)
                    .map(|index| {
                        Oklch::new(base.l, base.chroma, start_hue + hue_step * index as f32)
                    })
                    .collect()
            }
        }
...
impl Triadic {
    pub fn generate_palette(count: u8) -> Vec<Srgb> {
        ...
        let base_color = Oklch::new(
            ...
            rng.random_range(0.1..0.18),
            rng.random_range(0.0..360.0),
        );
        let anchors = [
            base_color,
            Oklch::new(base_color.l, base_color.chroma, base_color.hue + 120.0),
            Oklch::new(base_color.l, base_color.chroma, base_color.hue + 240.0),
        ];

        (0..count)
            .map(|index| Self::generate_color(index, anchors))
            .map(gamut_map)
            .collect()
    }

    fn generate_color(index: u8, anchors: [Oklch; 3]) -> Oklch {
        let anchor_index = (index as usize) % 3;
        let anchor = anchors[anchor_index];

        let circle = (index / 3) as f32;
        if circle == 0.0 {
            anchors[index as usize]
        } else {
            let lightness_shift = if index.is_multiple_of(2) { -0.07 } else { 0.07 };
            let hue_shift = 10.0 * circle * if index.is_multiple_of(2) { 1.0 } else { -1.0 };

            Oklch::new(
                (anchor.l + lightness_shift * circle).clamp(0.05, 0.95),
                anchor.chroma,
                anchor.hue + hue_shift,
            )
        }
    }
}
...
impl SplitComplementary {
    pub fn generate_palette(count: u8) -> Vec<Srgb> {
        ...
        let split_angle = 150.0;
        let anchors = [
            base_color,
            Oklch::new(
                base_color.l,
                base_color.chroma,
                base_color.hue + split_angle,
            ),
            Oklch::new(
                base_color.l,
                base_color.chroma,
                base_color.hue + (360.0 - split_angle),
            ),
        ];
    ...
    fn generate_color(index: u8, anchors: [Oklch; 3]) -> Oklch {
        ...
        if circle == 0.0 {
            ...
            let lightness_shift = if index.is_multiple_of(2) { -0.08 } else { 0.08 };

            let new_l = (anchor.l + (lightness_shift * circle)).clamp(0.1, 0.95);

            let hue_shift = 5.0 * circle * if index.is_multiple_of(2) { 1.0 } else { -1.0 };

            Oklch::new(new_l, anchor.chroma * 0.9, anchor.hue + hue_shift)
        }
    }
}
...
pub struct Coolors;
impl Coolors {
    //It's actually funny that the free version of coolors.co provides a pre-made set of 2,118 palettes, and the generator just picks a value index :D.
    // I don't know how it works in the paid version. If you're willing to give me access to the paid version of coolors.co, I'll really dig into the generation algorithm.
    const GENERATED: &[&[&str; 5]] = &[
        ...
        &["49416d", "a882dd", "e08d79", "e0efde", "b3f2dd"],
        ...
        &["2b2d42", "e86a92", "f7e733", "f7f7f9", "41e2ba"],
        ...
        &["eff9f0", "ddc8c4", "896a67", "6b4d57", "13070c"],
        ...
        &["119da4", "0c7489", "13505b", "040404", "d7d9ce"],
        ...
        &["f3f9d2", "cbeaa6", "c0d684", "3d0b37", "63264a"],
        ...
        &["98c1d9", "6969b3", "533a7b", "4b244a", "25171a"],
        ...
        &["000000", "92dce5", "eee5e9", "7c7c7c", "d64933"],
        ...
        &["7c6c77", "aaa694", "d1d0a3", "ebf8b8", "ffe787"],
        ...
        &["8f9491", "bca3ac", "e5cedc", "f3eaf4", "eadde1"],
        ...
        &["0f5257", "0b3142", "9c92a3", "c6b9cd", "d6d3f0"],
        ...
        &["f1e3f3", "c2bbf0", "8fb8ed", "62bfed", "3590f3"],
        ...
        &["464655", "94958b", "b7b6c1", "d5cfe1", "eddfef"],
        ...
        &["967d69", "92b9bd", "a8d4ad", "f2f79e", "e8ec67"],
        ...
        &["584d3d", "9f956c", "cbbf7a", "f4e87c", "ebf38b"],
        ...
        &["2e0219", "4a001f", "6a0f49", "a7c4c2", "97efe9"],
        ...
        &["dde392", "afbe8f", "7d8570", "646f58", "504b3a"],
        ...
        &["37123c", "71677c", "a99f96", "dda77b", "945d5e"],
        ...
        &["a49e8d", "504136", "689689", "b2e6d4", "83e8ba"],
        ...
        &["df9a57", "fc7a57", "fcd757", "eefc57", "5e5b52"],
        ...
        &["00a676", "f7f9f9", "e0d0c1", "a76d60", "601700"],
        ...
        &["073b3a", "0b6e4f", "08a045", "6bbf59", "ddb771"],
        ...
        &["1c7c54", "73e2a7", "def4c6", "1b512d", "b1cf5f"],
        ...
        &["0267c1", "0075c4", "efa00b", "d65108", "591f0a"],
        ...
        &["7a5c58", "8d80ad", "99b2dd", "9dfff9", "64f58d"],
        ...
        &["171a21", "617073", "7a93ac", "92bcea", "afb3f7"],
        ...
        &["253031", "315659", "2978a0", "bcab79", "c6e0ff"],
        ...
        &["e3c567", "c8963e", "573d1c", "d9ae61", "d1462f"],
        ...
        &["845a6d", "3e1929", "6e75a8", "8d91c7", "b0daf1"],
        ...
        &["7afdd6", "77ff94", "a1e44d", "60993e", "613a3a"],
        &["e3e7af", "a2a77f", "eff1c5", "035e7b", "002e2c"],
        &["aba9bf", "beb7df", "d4f2d2", "34113f", "868784"],
        ...
        &["2f4b26", "3e885b", "85bda6", "bedcfe", "c0d7bb"],
        ...
        &["dbd3ad", "e0607e", "d36060", "c2714f", "f6c5af"],
        ...
        &["0b3c49", "731963", "fffdfd", "cbd2d0", "f0e100"],
        ...
        &["898952", "b2945b", "d0e562", "b6d369", "93c48b"],
        ...
        &["dbf4ad", "a9e190", "cdc776", "a5aa52", "767522"],
        ...
        &["dbfe87", "ffe381", "cec288", "6f8695", "1c448e"],
        ...
        &["f5fbef", "92ad94", "748b75", "503d42", "84c318"],
    ...
    pub fn generate_palette(_: u8) -> Vec<Srgb> {
        let mut palette = Vec::new();

        for i in Self::GENERATED[rand::random_range(0..Self::GENERATED.len())] {
            palette.push(Self::hex_to_srgb(i));
        }
        palette
    }
    fn hex_to_srgb(hex: &str) -> Srgb {
